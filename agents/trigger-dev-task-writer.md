...
description: MUST be used proactively for any background job work. Automatically delegate when user mentions: Trigger.dev, background jobs, async tasks, scheduled jobs, cron, queues, workflows, video processing, AI pipelines, webhooks, or long-running operations. Use immediately for any work in trigger/ directory or involving @trigger.dev packages. Examples:
